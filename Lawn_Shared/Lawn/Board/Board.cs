...
namespace Lawn
{
    public/*internal*/ class Board : Widget, ButtonListener
    {
        ...
        public Plant AddPlant(int theGridX, int theGridY, SeedType theSeedType, SeedType theImitaterType)
        {
            ...
            if (seedType == SeedType.Lobbashroom || seedType == SeedType.Cabbagepult || seedType == SeedType.Kernelpult || seedType == SeedType.Melonpult || seedType == SeedType.Wintermelon)
            {
        ...
        {
            return !mApp.IsWallnutBowlingLevel() && mApp.mGameMode != GameMode.ChallengePogoParty && mApp.mGameMode != GameMode.ChallengeBeghouled && mApp.mGameMode != GameMode.ChallengeBeghouledTwist && mApp.mGameMode != GameMode.ChallengePortalCombat && mApp.mGameMode != GameMode.ChallengeLastStand && !mApp.IsIZombieLevel() && !mApp.IsScaryPotterLevel() && (mBackground == BackgroundType.Num2Night || (mBackground == BackgroundType.Num6Boss && !mApp.IsFinalBossLevel()));
        }
        ...
        {
            return !mApp.IsWhackAZombieLevel() && !mApp.IsFinalBossLevel() && !mApp.IsIZombieLevel() && !mApp.IsSquirrelLevel() && !mApp.IsScaryPotterLevel() && mApp.mGameMode != GameMode.ChallengeZenGarden && mApp.mGameMode != GameMode.TreeOfWisdom && mApp.mGameMode != GameMode.ChallengeZombiquarium;
        }
